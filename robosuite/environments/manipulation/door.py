...
class Door(SingleArmEnv):
    ...
    def reward(self, action=None):
        ...
        # print("self.sim.data.qpos[self.handle_qpos_addr]:", self.sim.data.qpos[self.handle_qpos_addr])

        # Scale reward if requested
        if self.reward_scale is not None:
            reward *= self.reward_scale / 1.0
        # print("environments/manipulation/door.py---reward:", reward)

        #todo: compute cost
        obstacle_pos = self.sim.data.get_geom_xpos("Door_panel")[0]

        # obstacle_pos=self.sim.data.qpos[self.door_handle_site_id]
        # print("obstacle_pos:", obstacle_pos)
        xdist = obstacle_pos - self._eef_xpos
        # print("self._eef_xpos:", self._eef_xpos)
        # print("-----xdist:", xdist)
        # obj_cost = int(np.abs(xdist) < 0.01)
        # print("-----xdist[1]:", xdist[1])
        if (np.abs(xdist[1]) < 0.001):
            obj_cost = 1
        else:
            obj_cost = 0
        if obj_cost > 0:
            self.sim.model.geom_rgba[5] = [1.0, 0, 0, 1.0]
            # self.door.important_sites["handle"].geom_rgba[0] = [1.0, 0, 0, 1.0]
            # self.sim.model.site_name2id.geom_rgba[0] = [1.0, 0, 0, 1.0]
        else:
            self.sim.model.geom_rgba[5] = [1, 1, 1, 1]
            # self.sim.model.site_name2id.geom_rgba[0] = [1.0, 0, 0, 1.0]
            # self.door.important_sites["handle"].geom_rgba[0] = [1.0, 0, 0, 1.0]
            # self.sim.model.site_name2id(self.door.important_sites["handle"]).geom_rgba[0]= [1.0, 0.5, 0.5, .8]
            # self.sim.model.geom_rgba[5] = [1.0, 0.5, 0.5, .8]
        cost = obj_cost
        # print("----cost:", cost)
    ...
    def _load_model(self):
        ...
        print("environments/manipulation/door.py--table-xpos:", xpos)
        print("environments/manipulation/door.py--table-self.robots[0].robot_model.set_base_xpos(xpos):", self.robots[0].robot_model.set_base_xpos(xpos))
    ...
    def _setup_references(self):
        ...
        # panel_data = self.sim.model.get_geom_xpos("panel")[0]
        # panel_data = self.sim.model.body_name2id(self.door.panel_geom)
        # print("++++++++++++panel_data+++++++++:",panel_data)

        print("self.sim.model.get_joint_qpos_addr(self.door.joints[0]):", self.sim.model.get_joint_qpos_addr(self.door.joints[0]))
        print("self.sim.model.get_joint_qpos_addr(self.door.joints[1]):", self.sim.model.get_joint_qpos_addr(self.door.joints[1]))
        print("self.sim.model.site_name2id(self.door.important_sites[handle]):", self.sim.model.site_name2id(self.door.important_sites["handle"]))
        ...
        print("door----self.object_body_ids:",[self.object_body_ids["door"]])
        print("handle------self.sim.data.qpos[self.door_handle_site_id]:", self.sim.data.qpos[self.door_handle_site_id])
