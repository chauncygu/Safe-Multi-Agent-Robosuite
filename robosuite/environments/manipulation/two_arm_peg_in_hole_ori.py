...
class TwoArmPegInHole(TwoArmEnv):
    ...
    def __init__(
        ...
        # self.model = load_model_from_path("/home/andrew/MyCode20201108/safe_robot/safe_env_robosuite/envs/robosuite/robosuite/models/assets/objects/plate-with-hole.xml")
        # self.sim = MjSim(model=self.model)
        #xml_string = "/home/andrew/MyCode20201108/safe_robot/safe_env_robosuite/envs/robosuite/robosuite/models/assets/objects/plate-with-hole.xml"
        # xml_string = None
    ...
    def reward(self, action=None):
        ...
        if self.reward_shaping:
            ...
            # print("self.sim.data.body_xpos:", self.sim.data.body_xpos)
            print("hole_pos:", hole_pos)
            # print("self.sim.data.hole_obstacle_frame0-----:", self.sim.data.get_geom_xpos("hole_obstacle_frame0"))
            print("self.sim.data.hole_obstacle_frame1-----:", self.sim.data.get_geom_xpos("hole_obstacle_frame1"))
            # print("self.sim.data.hole_obstacle_frame2-----:", self.sim.data.get_geom_xpos("hole_obstacle_frame2"))
            # print("self.sim.data.hole_obstacle_frame3-----:", self.sim.data.get_geom_xpos("hole_obstacle_frame3"))
            # dist00 = np.linalg.norm(self.sim.data.get_geom_xpos("hole_obstacle_frame0") - hole_pos)
            # dist10 = np.linalg.norm(self.sim.data.get_geom_xpos("hole_obstacle_frame1") - hole_pos)
            # dist20 = np.linalg.norm(self.sim.data.get_geom_xpos("hole_obstacle_frame2") - hole_pos)
            # dist30 = np.linalg.norm(self.sim.data.get_geom_xpos("hole_obstacle_frame3") - hole_pos)
            # # print("dist00++++++:", dist00)
            # print("dist10++++++:", dist10)
            # print("dist20++++++:", dist20)
            # print("dist30++++++:", dist30)
            ...
            # dist0 = np.linalg.norm(self.sim.data.get_geom_xpos("hole_obstacle_frame0") - gripper_site_pos)
            dist1 = np.linalg.norm(self.sim.data.get_geom_xpos("hole_obstacle_frame1") - gripper_site_pos)
            dist2 = np.linalg.norm(self.sim.data.get_geom_xpos("hole_obstacle_frame2") - gripper_site_pos)
            dist3 = np.linalg.norm(self.sim.data.get_geom_xpos("hole_obstacle_frame3") - gripper_site_pos)
            # print("dist0-----:", dist0)
            # print("dist1-----:", dist1)
            # print("dist2-----:", dist2)
            # print("dist3-----:", dist3)
            #
            #
            # # print("gripper_site_pos:", gripper_site_pos)
            # print("dist:", dist)
        ...
        # compute cost
        # self.myMujoco = MujocoEnv()
        # print("self.hole_obstacle_frame1_obstacle_pos========my", self.myMujoco.hole_obstacle_frame1_obstacle_pos)
        t, d, cos = self._compute_orientation()
        # t (float): parallel distance
        # d (float): perpendicular distance
        # cos (float): angle
        # if d < 0.03 or t < -0.12 or t > 0.14:
        #     cost = 1

        if self._check_success() is not True:
            print("do not success!!!!!")
            if dist1 < 0.15 or dist2 < 0.15 or dist3 < 0.15:
                cost = 1.0



        if cost > 0:
            self.sim.model.geom_rgba[5] = [1.0, 1.0, 0, 1.0]
        else:
    ...
    def _load_model(self):
        ...
        # Load hole object
        # wallpos = self.hole.get_geom_xpos("obstacle_frame0")[0]
        # self.sim.forward()

        # self.hole_obstacle_frame1_obstacle_pos = self.sim.data.get_geom_xpos("hole_obstacle_frame1")

        hole_obj = self.hole.get_obj()
        print("hole_obj========", hole_obj)
        # wallpos = hole_obj.get("bottom_site")
        # obstacle_pos = self.sim.data.get_geom_xpos("obstacle_frame0")[0]
        # print("obstacle_frame0--------------", obstacle_pos)
