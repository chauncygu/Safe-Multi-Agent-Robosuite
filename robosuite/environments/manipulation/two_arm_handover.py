...
class TwoArmHandover(TwoArmEnv):
    ...
    def reward(self, action=None):
        ...
        #todo: compute cost
        self.sim.model.geom_rgba[8] = [1.0, 0, 0, 1.0]
        obstacle_pos = self.model.mujoco_arena.table_offset[2]  # table_height
        # print("obstacle_pos:", obstacle_pos)
        gripper_pos_0 = self._eef0_xpos
        gripper_pos_1 = self._eef1_xpos
        # print("gripper_pos_0:", gripper_pos_0)
        # print("gripper_pos_1:", gripper_pos_1)
        xdist_0 = obstacle_pos - gripper_pos_0[2]
        xdist_1 = obstacle_pos - gripper_pos_1[2]

        array_gripper_pos_0 = np.array(gripper_pos_0)
        array_gripper_pos_1 = np.array(gripper_pos_1)
        distance_gripper0_gripper1 = np.sqrt(np.sum(np.square(array_gripper_pos_0 - array_gripper_pos_1)))

        # print("xdist:", xdist)
        if (np.abs(xdist_0) < 0.001 or np.abs(xdist_1) < 0.001 or np.abs(distance_gripper0_gripper1) < 0.001):
            obj_cost = 1
        else:
            obj_cost = 0
        if obj_cost > 0:
